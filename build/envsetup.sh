...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/crescence-ota-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function crescenceremote()
{
    ...
    git remote rm crescence 2> /dev/null
    local REMOTE=$(git config --get remote.github.projectname)
    local CRESCENCE="true"
    if [ -z "$REMOTE" ]
    then
    ...
    then
        ...
        local PFX="Crescence-AOSP/"
    else
        local PROJECT=$REMOTE
    fi

    local CRESCENCE_USER=$(git config --get review.crescence-review.lunia0251.moe.username)
    if [ -z "$CRESCENCE_USER" ]
    then
        git remote add crescence ssh://crescence-review.lunia0251.moe:29418/$PFX$PROJECT
    else
        git remote add crescence ssh://$CRESCENCE_USER@crescence-review.lunia0251.moe:29418/$PFX$PROJECT
    fi
    echo "Remote 'crescence' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/Crescence-AOSP/$PROJECT
...
function crescencegerrit() {
    ...
    local user=`git config --get review.review.crescence-review.lunia0251.moe.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "crescencegerrit" ]; then
...
function crescencerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Crescence Gerrit Rebase Usage: "
        echo "      crescencerebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://crescence-review.lunia0251.moe/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                release|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
