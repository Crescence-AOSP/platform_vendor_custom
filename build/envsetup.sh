...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/rwiceAOSP-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function crescentremote()
{
    ...
    git remote rm crescent 2> /dev/null
    ...
    then
        ...
        local PFX="crescentUI/"
    else
        local PROJECT=$REMOTE
    fi

    local CRESCENT_USER=$(git config --get review.crescent-review.duckdns.org.username)
    if [ -z "$CRESCENT_USER" ]
    then
        git remote add crescent ssh://review.crescent-review.duckdns.org:29418/$PFX$PROJECT
    else
        git remote add crescent ssh://$CRESCENT_USER@review.crescent-review.duckdns.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'crescent' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/crescentUI/$PROJECT
...
function crescentgerrit() {
    ...
    local user=`git config --get review.review.crescent-review.duckdns.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "crescentgerrit" ]; then
...
function crescentrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "crescentUI Gerrit Rebase Usage: "
        echo "      crescentrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://crescent-review.duckdns.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
